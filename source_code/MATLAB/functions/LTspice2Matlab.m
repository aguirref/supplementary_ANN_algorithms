    ...
    if strcmpi(file_format, 'binary')
        ...
        if strcmpi( simulation_type, '.tran' ) || strcmpi( simulation_type, '.dc' ) || strcmpi( simulation_type, '.noise' )
            ...
            if step_flag
                ...
                starting_points=find( raw_data.time_vect == raw_data.time_vect(1) );
                
                max_sizeof_data=0;
                for idx_data=2:length(starting_points)
                    sizeof_data=(starting_points(idx_data)-1)-starting_points(idx_data-1);
                    if sizeof_data>max_sizeof_data
                        max_sizeof_data=sizeof_data;
                    end    
                end
                sizeof_data=NumPnts-starting_points(end);
                if sizeof_data>max_sizeof_data
                    max_sizeof_data=sizeof_data;
                end 
                
                for idx_data=1:length(starting_points)
                    if idx_data<length(starting_points)
                        data_time=raw_data.time_vect(:,starting_points(idx_data):starting_points(idx_data+1)-1);
                        variable_data=raw_data.variable_mat(:,starting_points(idx_data):starting_points(idx_data+1)-1);
                        diff_data_time=diff(raw_data.time_vect(:,starting_points(idx_data):starting_points(idx_data+1)-1));
                    else
                        data_time=raw_data.time_vect(:,starting_points(idx_data):end);
                        variable_data=raw_data.variable_mat(:,starting_points(idx_data):end);
                        diff_data_time=diff(raw_data.time_vect(:,starting_points(idx_data):end));                        
                    end
                    [out,idx]=max(diff_data_time);
                    
                    points2add=max_sizeof_data-size(data_time,2)+1;
                    for points2add_counter=1:points2add
                        data_time=[data_time(1:idx) (data_time(idx)+data_time(idx+1))./2 data_time(idx+1:end)];
                        variable_data=[variable_data(:,1:idx) (variable_data(:,idx)+variable_data(:,idx+1))./2 variable_data(:,idx+1:end)];
                        diff_data_time=diff(data_time);
                        [out,idx]=max(diff_data_time);
                    end
                    if exist('data_time_accu','var')
                        data_time_accu=horzcat(data_time_accu,data_time);
                        variable_data_accu=horzcat(variable_data_accu,variable_data);
                    else
                        data_time_accu=data_time;
                        variable_data_accu=variable_data;
                    end
                end
                raw_data.variable_mat=variable_data_accu;
                raw_data.time_vect=data_time_accu;
